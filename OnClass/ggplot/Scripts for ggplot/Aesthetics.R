...
ggplot(iris,aes(x = Sepal.Length,y=Sepal.Width)) +   # aes实际发挥作用，在第三层被调用
  geom_point(col = "red") +
  geom_jitter(col = "blue")

ggplot(iris) + 
  geom_point(aes(x = Sepal.Length,y=Sepal.Width,col=Petal.Length))
...
ggplot(mtcars, aes(x = wt, y = mpg, fill = cyl)) +
  geom_point(shape = 21, alpha = 0.6, size = 4,fill = "red")

# 3 - Map am to col in the above plot
ggplot(mtcars, aes(x = wt, y = mpg, fill = as.factor(cyl), col = am)) +
...
ggplot(iris,aes(x=Sepal.Length,y=Sepal.Width,col=Species)) +
  ...
  scale_color_discrete("Species Set",
                       labels = c("Setosa 1","Versicolour 2","Virginica 3"))
#labs
ggplot(iris,aes(x=Sepal.Length,y=Sepal.Width,col=Species)) +
  geom_point(position="jitter")+
